 ...
 void DisplayHelp()
 {
    ...
    printf("stat : To Display infomation of file using file descriptor\n");
 ...
 int WriteFile(int fd,char *arr,int isize)
 {
    ...
    strncpy((UFDTArr[fd].ptrfiletable->ptrinode->Buffer)+(UFDTArr[fd].ptrfiletable->writeoffset),arr,isize);        //strncpy(kashat,kay copykaraych,kiti copy karaycch)
...
int main()
{
    ...
    while(1)
    {
        ...
        count=sscanf(str,"%s%s%s%s",command[0],command[1],command[2],command[3]);       //string madhun input ghe
        ...
        else if(count==2)
        {
            if(strcmp(command[0],"stat")==0)
            {
                ...
                if(ret==-1)
                    printf("ERROR : Incorrect patameters\n");
                if(ret==-2)
        ...
        else if(count==3)
        {
            if(strcmp(command[0],"create")==0)
            {
                ...
                if(ret>=0)
                    printf("File is successfully ceate with file descriptor : %d\n",ret);
                if(ret==-1)
                    printf("ERROR : Incorrect parameters\n");
                if(ret==-2)
